...
* Making requests to publish D-TROS to the service (publisher applications only)
* Making requests to retrieve data from the service

.. inset::

    This guide assumes you have already created a publisher/consumer application, and know your API key and secret. You can create applications and view application credentials in the D-TRO User Portal
...
* The integration environment is designed to allow users of the service to test their application's ability to publish to/consume from the service, using test data
...
The integration and production environments are two isolated services, and as such are accessed at different URLs.

* Integration: https://dtro-integration.dft.gov.uk/v1
* Production: https://dtro.dft.gov.uk/v1

Because of this, separate applications are required for interfacing with each environment.

Authenticating Your Application
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Each request to the D-TRO service must contain an **access token** supplied as a request header in order for the service to authenticate the request. The service makes use of the **OAuth 2.0 client credential flow** for authentication, where a **client ID** and **client secret** are exchanged for an **access token**. This token permits an application to interface with the service for a fixed period of time, util the token expires. Token expiry os configred for **30 minutes**, after which a new token will need to be issued through the same flow.
...
There are two ways to publish a D-TRO the the service. These are:

1. Sending JSON in the body of a ``POST`` request
2. Sending a JSON file in a ``POST`` request

.. inset::

    Only applications with the publisher scope can publish D-TROs to the service. If you try and publish a D-TRO using a consumer account, you will receive an error response.

Submitting JSON in the payload body
***********************************
...
The response payload contains a single field, ``id``, which is the unique ID of the created D-TRO record.

.. inset::
    There is a 10MB request payload limit imposed by the API proxy. If the request body exceeds this limit, the submission will be rejected. Consider minifying your JSON payload, or uploading as a gzip-compressed file (see section below).
