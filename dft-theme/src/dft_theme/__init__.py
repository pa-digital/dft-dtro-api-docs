...
def inject_custom_toc(app, pagename, templatename, context, doctree):
    ...
    toc_root = toctree.get_toctree_for(app.config.master_doc, app.builder, collapse=False, maxdepth=1)
    if toc_root is None:
...
def add_links(app, pagename, templatename, context, doctree):
    with open(os.path.join(os.path.dirname(__file__), "links.json"), "r", encoding="utf-8") as f:
        context["links"] = json.load(f)

def setup(app):
    from .extensions import button, button_nav, notification, toc
