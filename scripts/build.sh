...
docker exec -e PR_NUMBER=$PR_NUMBER $(docker ps --filter="name=api-documentation" -q) \
  bash -c "redocly build-docs specs/openapi.yaml -o source/_static/redoc.html && make html"

docker exec -e PR_NUMBER=$PR_NUMBER $(docker ps --filter="name=user-portal-documentation" -q) \
  bash -c "make html"

docker exec -e PR_NUMBER=$PR_NUMBER $(docker ps --filter="name=landing" -q) \
  bash -c "make html"

sphinx-multiversion data-model-user-guide/source data-model-user-guide/build
